...
const EditorProfile = () => {
    ...
    const getEditor = async () => {
        setLoading(true);
        try {
            const response = await fetch(apiList.geteditor, {
                method: 'GET',
                headers: {
            ...
            toast.warn(`EditorProfile :  ${err.message}`);
        }
                        ...
                        <div className='border bg-blue-100 max-w-lg lg:h-48 mx-auto rounded-lg flex items-center justify-center p-2 relative'>
                            ...
                            <div className='w-full py-4'>
                                <div className='flex items-center justify-center flex-col max-lg:pt-12 text-center mb-4'>
                                    <div className='text-xl lg:text-2xl font-semibold'>{editor.name}</div>
                                </div>

                                <div className='text-center'>
                                    <div><span className='font-semibold'>Contact Number : </span><span>{editor.contactnum}</span></div>
                                </div>
                            </div>
